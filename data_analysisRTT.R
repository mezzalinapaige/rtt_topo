...
plot_tstats <- function(df1, df2, filename1, ylimitNeg, filename2) {
  ...
  #save to PDF
  aspectR2 <- 5
  pdf(file = filename2, onefile = TRUE, height = 4.8, width = 4.8*aspectR2, useDingbats = FALSE)
  plot(bothplot)
  dev.off()
}
...
run_all <- function(df) {
  ...
  #Plot lineplots 
  plot_tstats(SD_noWeightsDF, GD_noWeightsDF, "line_NoWeights.pdf", -55, "point_NoWeights.pdf")
  plot_tstats(SD_taxonWeightsDF, GD_taxonWeightsDF, "line_TaxaWeights.pdf", -85, "point_TaxaWeights.pdf")
  plot_tstats(SD_branchWeightsDF, GD_branchWeightsDF, "line_BranchWeights.pdf", -85, "point_BranchWeights.pdf")
}
